...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  const twoTo = [];
  arr.forEach(num => twoTo.push(Math.pow(2, num)));
  return twoTo;
};
...
const charCode = (arr) => {
  // Solution code here...
  return arr.map(lett => lett.charCodeAt());
};
...
const evenOdd = (arr) => {
  // Solution code here...
  return arr.map(elm => typeof elm === 'number' ? elm % 2 === 0 ? 'even' : 'odd' : 'N/A');
};
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map(stat => { return { name: stat.stat.name, total: stat.effort + stat.baseStat }; });
};
