...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  return starWarsArr.sort((a, b) => {
    ...
    if (Number(a.height) > Number(b.height)) return -1;
    if (Number(a.height) === Number(b.height)) return 0;
  });
};
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let res = recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ') + 1);
    result.push(res.slice(res.indexOf(' ') + 1));
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  string.splice(str.length - numberOfCharacters, numberOfCharacters);
  return string.join('');
};
...
const removeVowels = (str) => {
  ...
  const arr = str.split('');
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] === 'e' || arr[i] === 'o' || arr[i] === 'a' || arr[i] === 'i' || arr[i] === 'u') {
...
const extractVowels = (str) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] === 'e' || arr[i] === 'o' || arr[i] === 'a' || arr[i] === 'i' || arr[i] === 'u') {
      removed.push(arr.splice(arr.indexOf(arr[i]), 1));
      --i;
    }
  }
  return [arr.join(''), removed.sort().join('')];
};
