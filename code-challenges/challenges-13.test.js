...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(num => num.split('(').join('').split(')').join('').split('-').join('').split(' ').join(''));
  // return arr.map(num => num.substring(1, 4) + num.substring(6, 9) + num.substring(10, 14));
  // return arr.map(num => num.slice(1, 4) + num.slice(6, 9) + num.slice(10, 14));
};
...
const allHappy = (arr) => {
  // Solution code here...
  let comp = arr.filter(str => str.includes(':)'));
  return comp.length === arr.length;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  let comp = arr.filter(str => str.includes(target));
  return comp.length === arr.length;
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(nArr => nArr.filter(name => !name.includes('Brook')));
};
...
const sortByDay = (arr) => {
  // Solution code here...
  const result = [[], [], [], [], [], [], []];
  arr.forEach(event => daysOfWeek.forEach((day, i) => {
