...
const getNames = (arr) => {
  // Solution code here...
  return arr.map(obj => obj.name.split('').reduce((acc, lett) => lett += acc, ''));
};
...
const appendFirstToLast = (arr) => {
  // Solution code here...
  arr.forEach((num, i) => { if (i === 0) arr.push(num); });
};
...
const setStatusAsAuthor = (people) => {
  // Solution code here...
  people.forEach(per => per.isAuthor = true);
};
...
const append = (arr1, arr2) => {
  // Solution code here...
  arr2.forEach(elm => arr1.push(elm));
};
