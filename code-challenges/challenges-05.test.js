...
const reversedString = (str) => {
  ...
  const arr = str.split('').reduce((acc, lett) => {
    acc[idx] = lett;
    idx = idx - 1;
    return acc;
  }, []);
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((acc, char) => {
    if (char.children) acc += char.children.length;
    return acc;
  }, 0);
};
...
const extractStat = (statName, arr) => {
  ...
  return arr.reduce((acc, elm) => {
    if (statName === elm.stat.name) acc = elm;
    return acc;
  }, null);
};
...
const extractChildren = (arr) => {
  // Solution code here...
  return arr.filter(obj => obj.name.includes('a')).reduce((acc, obj) => {
    if (obj.children) obj.children.forEach(child => acc.push(child));
    return acc;
  }, []);
};
