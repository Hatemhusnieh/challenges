...
const updateAnimal = (arr, callback) => {
  // Solution code here...
  return arr.map((elm) => callback(elm));
};
...
const sortMeetingsByDay = (arr) => {
  ...
  return arr.sort((a, b) => {
    if (order[a.dayOfWeek] > order[b.dayOfWeek]) return 1;
    if (order[a.dayOfWeek] < order[b.dayOfWeek]) return -1;
    if (order[a.dayOfWeek] === order[b.dayOfWeek]) return 0;
  });
};
...
const sortSchedule = (arr) => {
  ...
  return arr.sort((a, b) => {
    ...
    if (order[a.dayOfWeek] === order[b.dayOfWeek]) {
      ...
      if (Number(a.end) - Number(a.start) < Number(b.end) - Number(b.start)) return -1;
      if (Number(a.end) - Number(a.start) === Number(b.end) - Number(b.start)) return 0;
    }
  });
};
