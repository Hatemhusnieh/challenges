...
const count = (target, input) => {
  ...
  return input.reduce((acc, arr) => {
    return acc += arr.reduce((acc2, int) => {
      if (int === target) acc2++;
      return acc2;
    }, 0);
  }, 0);
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, arr) => {
    return acc += arr.reduce((acc2, num) => {
      return acc2 += num;
    }, 0);
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  // const array = input.reduce((acc, arr) => {
  //   acc.push(arr.reduce((acc2, num) => {
  //     if (typeof num === 'number' && num % 5 === 0) acc2.push(num);
  //     return acc2;
  //   }, []));
  //   return acc;
  // }, []);
  ...
  return filtered.map(arr => arr.map(num => Math.pow(2, num)));
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findMaleAndFemale = (data) => {
  ...
  return data.reduce((acc, obj, idx) => {
    if (obj.gender === 'male' || obj.gender === 'female') {
      if (idx !== data.length - 1) acc += `${obj.name} and `;
      else acc += `${obj.name}`;
    }
    return acc;
  }, '');
