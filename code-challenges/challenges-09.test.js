...
const updateNumbers = (obj) => {
  // Solution code here...
  return Object.entries(obj).map(elm => `${elm[0]}: ${elm[1]}`);
};
...
const hasChildrenValues = (arr, character) => {
  ...
  return arr.reduce((acc, char) => {
    if (char.name === character) {
      if (Object.values(char).length === 4) acc = true;
    }
    return acc;
  }, false);
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  return arr.reduce((acc, char) => {
    if (char.name === character) {
      if (Object.entries(char)[2][0] === 'children' && Object.entries(char)[2][1].length > 0) acc = true;
    }
    return acc;
  }, false);
};
...
const totalCharacters = (arr) => {
  ...
  return arr.reduce((acc, obj) => {
    if (obj.name) acc++;
    if (obj.spouse) acc++;
    if (obj.children) acc += obj.children.length;
    return acc;
  }, 0);
};
...
Write a function named houseSurvivors. You may modify your houseSize function from challenge 9 to use as the basis of this function.
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(obj => {
    let members = 1;
    if (!deceasedSpouses.includes(obj.spouse)) members++;
    if (obj.children) members += obj.children.length;
    survivors.push({
      house: obj.house,
      members,
